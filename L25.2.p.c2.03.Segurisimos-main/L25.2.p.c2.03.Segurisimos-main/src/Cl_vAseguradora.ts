...
export default class Cl_vAseguradora extends Cl_vGeneral {
  private _vVida: Cl_vVida;
  private _vVehiculo: Cl_vVehiculo;
  private dataPolizas: HTMLElement;
  private lblTotalPagado: HTMLElement;
  private lblCantidadPolizas: HTMLElement;
  private lblPromedioMontoPagado: HTMLElement;
  ...
  reportarPoliza({
    ...
    this.dataPolizas.innerHTML += `
      ...
      <td class="colCurrency">${`Bs.${dataPoliza.montoAsegurar.toFixed(
        2
      )}`}</td>
      ...
      <td class="colCurrency">${`Bs.${dataPoliza.montoCobertura.toFixed(
        2
      )}`}</td>
